...
def train():
    ...
    for epoch in trange(args.n_epochs, desc="Epochs"):
        ...
        for ts_batch, _, target_batch in tqdm(train_dl, desc="Batches", leave=False):
            ...
            with torch.no_grad():
                ...
                pred_raw = scaler.inverse_transform(pred.detach().cpu().numpy())

                abs_error = np.abs(target_raw - pred_raw)
                denominator = np.abs(target_raw) + np.abs(pred_raw) + 1e-8  # защита от деления на ноль
                smape = (abs_error / denominator).mean() * 100  # в процентах
                smape_values.append(float(smape))  # преобразуем numpy.float64 -> float

        avg_loss = np.mean(losses)
        avg_smape = np.mean(smape_values)
        print(f"Epoch {epoch+1}/{args.n_epochs}, MAE Loss: {avg_loss:.4f}, SMAPE: {avg_smape:.2f}%")
    ...
    test_metrics = test(
        ...
        use_threshold_from_val=True  # вычисляем порог на тестовых ошибках
    )
    import os
    os.makedirs('saved_models', exist_ok=True)
    torch.save(model.state_dict(), os.path.join('saved_models', f'ts_{args.name}_experiment.pt'))
